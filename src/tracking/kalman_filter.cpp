...
void KalmanFilter::setup_kalman_filter()
{
    ...
    kf_.processNoiseCov.at<float>(4, 4) = 0.1f;    // vx
    kf_.processNoiseCov.at<float>(5, 5) = 0.1f;    // vy
    kf_.processNoiseCov.at<float>(6, 6) = 0.001f;   // vw
    kf_.processNoiseCov.at<float>(7, 7) = 0.001f;   // vh
    kf_.processNoiseCov *= 1e-1;

    // Measurement noise covariance (R)
    kf_.measurementNoiseCov = cv::Mat::eye(measurement_size, measurement_size, CV_32F);
    kf_.measurementNoiseCov.at<float>(0, 0) = 0.05f;
    kf_.measurementNoiseCov.at<float>(1, 1) = 0.05f;
    kf_.measurementNoiseCov.at<float>(2, 2) = 0.02f;
    kf_.measurementNoiseCov.at<float>(3, 3) = 0.02f;
